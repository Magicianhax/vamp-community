...
export function ResourceComments({ resourceId, userId, isAdmin = false }: ResourceCommentsProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sessionError || !session) {
        if (sessionError) console.error('ResourceComments: Session error:', sessionError)
        setError('Please sign in to comment')
        setSubmitting(false)
        return
      }
      ...
      if (session.user.id !== userId) {
        console.error('ResourceComments: User ID mismatch:', session.user.id, 'vs', userId)
  ...
  const handleReply = async (e: React.FormEvent, parentId: string) => {
    ...
    try {
      ...
      const { data: { session }, error: sessionError } = await supabase.auth.getSession()
      if (sessionError || !session) {
        if (sessionError) console.error('ResourceComments Reply: Session error:', sessionError)
        setError('Please sign in to reply')
        setSubmitting(false)
        return
      }
      ...
      // Use session.user.id directly for reliability
      if (session.user.id !== userId) {
        console.error('ResourceComments Reply: User ID mismatch:', session.user.id, 'vs', userId)
        setError('Authentication failed. Please sign in again.')
        setSubmitting(false)
        return
      }
